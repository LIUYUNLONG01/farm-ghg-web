...
function createRegionalRow(row: LivestockRecord, index: number, standardVersion: StandardVersion, projectRegion: string) {
  const regional = getRegionalManureCH4Factor(standardVersion, projectRegion, row.species);
  return {
    ...
    parameterSourceLabel: regional ? `${standardVersion} 表C.7：${regional.regionGroup} / ${row.species}` : "待选择推荐因子或参数法",
    notes: regional ? `表C.7 区域化推荐因子已按初始化自动带入：${regional.regionGroup} / ${row.species}` : "",
  };
}
...
function syncRowsWithLivestock(
  ...
  const nextRows: Array<{
    ...
    sharePercent: number; vsKgPerHeadPerDay: number; boM3PerKgVS: number; mcfPercent: number;
  ...
  livestockRows.forEach((livestockRow, sourceIndex) => {
    ...
    if (parameterRows.length > 0) {
      parameterRows.forEach((row) => {
        ...
        if (shouldRefreshDefaults) {
          const matched = getManureCH4DefaultFactor(standardVersion, livestockRow.species, row.managementSystem ?? "", "20");
          if (matched) {
            nextRows.push({
    ...
    const regional = shouldRefreshDefault ? getRegionalManureCH4Factor(standardVersion, projectRegion, livestockRow.species) : null;
    ...
    nextRows.push({
      ...
      managementSystem: "", sharePercent: 100, vsKgPerHeadPerDay: 0, boM3PerKgVS: 0, mcfPercent: 0,
      ...
      parameterSourceLabel: shouldRefreshDefault
        ? (regional ? `${standardVersion} 表C.7：${regional.regionGroup} / ${livestockRow.species}` : "未匹配区域化推荐因子，需手动填写")
        : regionalRow.parameterSourceLabel ?? "手工输入",
      notes: shouldRefreshDefault
        ? (regional ? mergeNote(regionalRow.notes, `表C.7 区域化推荐因子已按当前养殖活动口径重新同步：${regional.regionGroup} / ${livestockRow.species}`) : regionalRow.notes ?? "")
        : regionalRow.notes ?? "",
    });
  });
...
export default function ManureCH4Page() {
  ...
  // ── handlers ──────────────────────────────────────────────────────────────
  const replaceRowsForSource = (sourceIndex: number, nextRowsForSource: ManureCH4FormInput["rows"]) => {
  ...
  const applyRegionalDefaultForSource = (sourceIndex: number) => {
    const livestockRow = livestockRows[sourceIndex];
    if (!livestockRow) return;
    const matched = getRegionalManureCH4Factor(standardVersion, projectRegion, livestockRow.species);
  ...
  const applyParameterDefaultForRow = (rowIndex: number) => {
    ...
    if (!row) return;
    const matched = getManureCH4DefaultFactor(standardVersion, row.species, row.managementSystem || "", "20");
    ...
    setValue(`rows.${rowIndex}.vsKgPerHeadPerDay`, matched.vsKgPerHeadPerDay, { shouldValidate: true });
    setValue(`rows.${rowIndex}.boM3PerKgVS`, matched.boM3PerKgVS, { shouldValidate: true });
    setValue(`rows.${rowIndex}.mcfPercent`, matched.mcfPercent, { shouldValidate: true });
  ...
  const onSubmit = (values: ManureCH4FormValues) => {
    const rows: ManureCH4Record[] = values.rows.map((row) => ({
      ...
      managementSystem: row.managementSystem?.trim() ? row.managementSystem.trim() : undefined,
      sharePercent: row.sharePercent, vsKgPerHeadPerDay: row.vsKgPerHeadPerDay,
      boM3PerKgVS: row.boM3PerKgVS, mcfPercent: row.mcfPercent,
      ...
      <main className="min-h-screen bg-gray-50 font-sans">
        ...
        <div className="mx-auto max-w-3xl px-6 py-16">
          <div className="rounded-2xl border border-green-100 bg-white p-8 shadow-sm">
            ...
            <p className="mt-3 text-sm text-gray-500 leading-7">先完成「基础信息」和「养殖活动数据」这两步，再进入粪污管理 CH₄ 模块。</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
                    ...
                    <div className="flex items-start justify-between gap-4 mb-5">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 pl-9">
                          年平均存栏：{fmt(groupPreview?.annualAverageHead)} · 该畜种 CH₄：{fmt(groupPreview?.totalCH4TPerYear)} t/yr
                        </p>
                      </div>
                      ...
                      <div className="rounded-xl border border-green-100 bg-gray-50 p-4">
                        ...
                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                          ...
                          <label className="block">
                            <span className="mb-1.5 block text-xs font-medium text-gray-500 uppercase tracking-wide">区域化推荐因子（kg CH₄/头·年）</span>
                            <input type="number" step="any"
                              {...register(`rows.${rowIndexes[0]}.regionalEmissionFactor`, { valueAsNumber: true, onChange: () => markRowManual(rowIndexes[0], "手工覆盖表 C.7 推荐因子") })}
          ...
          <div className="flex flex-wrap gap-3">
            <button type="submit" disabled={isSubmitting} className="px-6 py-2.5 rounded-xl bg-green-700 text-white text-sm font-medium shadow-sm hover:bg-green-900 disabled:opacity-50 disabled:cursor-not-allowed transition">
              {isSubmitting ? "保存中..." : "保存粪污管理 CH₄ 草稿"}
            </button>
            <Link href="/project/enteric" className="px-5 py-2.5 rounded-xl border border-green-100 bg-white text-sm font-medium text-green-800 shadow-sm hover:bg-green-50 transition">返回上一页</Link>
            <Link href="/project/manure-n2o" className="px-5 py-2.5 rounded-xl border border-green-200 bg-white text-sm font-medium text-green-700 shadow-sm hover:bg-green-50 transition">下一步：粪污管理 N₂O →</Link>
          </div>
        </form>
      </div>
    </main>
